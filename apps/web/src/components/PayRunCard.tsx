...
import { formatCurrency, formatLongDate, formatNumber, formatSlashDate } from '../lib/format'

export function PayRunCard({ payRun }: { payRun: CurrentPayRun }) {
  const hasPeriod = Boolean(payRun.periodStart && payRun.periodEnd)
  ...
  return (
    <div className="rounded-xl border-2 border-brand bg-white p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3">
          <h2 className="text-lg font-semibold text-ink">
            {hasPeriod
              ? `Process Pay Run for ${formatSlashDate(payRun.periodStart)} - ${formatSlashDate(payRun.periodEnd)}`
              : 'Process Pay Run'}
          </h2>
          {payRun.status && (
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-3">
        ...
        <Figure label="No. of Employees" value={formatNumber(payRun.employeeCount)} />
      </div>
      ...
      {payRun.periodEnd && (
        <div className="mt-6 flex items-center gap-2 text-sm text-muted">
          ...
          <span>
            Pay your employees on {formatSlashDate(payRun.periodEnd)}. Record it here once you made
            the payment.
          </span>
        </div>
      )}
...
function Figure({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-1 min-h-8 text-2xl font-bold text-ink">{value}</p>
    </div>
  )
}
